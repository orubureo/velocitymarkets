    ...
    <div
        ...
        <div class="mb-8 text-center">
            ...
            <p class="text-zinc-500 dark:text-zinc-400 text-sm mt-1">
                {{ __('Enter your email and password below to log in') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('login.store') }}" class="flex flex-col gap-5">
                ...
                <div class="flex items-center justify-between">
                    ...
                    @if (Route::has('password.request'))
                        <a href="{{ route('password.request') }}"
                            class="text-sm font-medium text-[#d4880c] hover:text-[#ffae11] transition-colors" wire:navigate>
                            {{ __('Forgot password?') }}
                        </a>
                    @endif
                </div>
            ...
            <div class="flex items-center">
                <input id="remember" type="checkbox" name="remember" {{ old('remember') ? 'checked' : '' }}
                    class="rounded border-zinc-300 text-[#d4880c] shadow-sm focus:ring-[#d4880c] dark:bg-zinc-900 dark:border-zinc-700">
        ...
        <div class="mt-6 text-sm text-center text-zinc-600 dark:text-zinc-400">
            ...
            <a href="{{ route('register') }}" class="font-medium text-[#d4880c] hover:text-[#ffae11] transition-colors"
                wire:navigate>
                {{ __('Sign up') }}
            </a>
        </div>
